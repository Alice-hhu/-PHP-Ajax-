$(function(){
  ...
  // 页面请求数据显示
  var tableName = "activity";// 表名个性化，以下都是共用
  // 获取当前页面的url
  ...
  if (urlParam) { // 如果有请求参数，则是请求topic详情
    ...
    $.post("php/topicList.php",{table:tableName},function(data){
      for(var index in data.data){
        var topicList = $("<li class='list-group-item'><a href='newsList.html?table="+tableName+"&id="+data.data[index].id+"'>"+data.data[index].title+"</a><div class='listContent'>"+data.data[index].content+"</div><div class='topicDate'><span class='glyphicon glyphicon-calendar'></span> "+data.data[index].date+"</div></li>");
